...
// アクセス制限
  ...
  if(ereg("admin/mainmenu.php", $_SERVER['PHP_SELF']))
  {
  ...
  if($input_type == "title")
  {
       ...
       ereg($PHP_SELF, $HTTP_REFERER))
    {
      $admin_query = "update wps_menu set display_name='${input_name}',"
                   . "active='${input_active}'"
                   . " where internal_name='menu_contents'";
      mysql_query($admin_query, $db);
    }
    header("Location: admin.php?mode=admin&action=mainmenu");
  }
...
// 表示順の変更
  ...
  elseif(preg_match("/^move/", $input_type))
  {
    if(ereg($PHP_SELF, $HTTP_REFERER))
    {
      ...
      // 前後のカテゴリを移動
      $admin_query = "update wps_menu_contents set weight=${admin_back}"
                   . " where weight=${admin_weight}";
      mysql_query($admin_query, $db);

      // 編集中のカテゴリを移動
      $admin_query = "update wps_menu_contents set weight=${admin_forward}"
                   . " where id=${input_id}";
      mysql_query($admin_query, $db);
    }
    header("Location: admin.php?mode=admin&action=mainmenu");
...
// ヘッダーの有効化
  ...
  elseif($input_type == "show")
  {
    if(ereg($PHP_SELF, $HTTP_REFERER))
    {
      $admin_query = "update wps_menu_contents set header='Y'"
                   . " where id=${input_id}";
      mysql_query($admin_query, $db);
    }
    header("Location: admin.php?mode=admin&action=mainmenu");
...
// ヘッダーの無効化
  ...
  elseif($input_type == "hide")
  {
    if(ereg($PHP_SELF, $HTTP_REFERER))
    {
      $admin_query = "update wps_menu_contents set header='N'"
                   . " where id=${input_id}";
      mysql_query($admin_query, $db);
    }
    header("Location: admin.php?mode=admin&action=mainmenu");
...
<tr>
  <td align="center">
<a href="admin.php?mode=admin&amp;action=mainmenu&amp;type=conflict">
順位
</a>
  </td>
...
_EOT_;
  ...
  $admin_query = "select * from wps_menu_contents order by weight";
  $admin_menu_data = mysql_query($admin_query, $db);
  while($admin_menu = mysql_fetch_assoc($admin_menu_data))
  {
...
_EOT_;
    ...
    {
      $page_content .= <<<_EOT_
<a href="admin.php?mode=admin&amp;action=mainmenu&amp;type=moveup&amp;id=${admin_menu['id']}&amp;weight=${admin_menu['weight']}">
▲
</a>
_EOT_;
    }
    ...
    {
      $page_content .= <<<_EOT_
<a href="admin.php?mode=admin&amp;action=mainmenu&amp;type=movedown&amp;id=${admin_menu['id']}&amp;weight=${admin_menu['weight']}">
▼
</a>
_EOT_;
    }
  ...
  <td align="center">
    <a href="admin.php?mode=admin&amp;action=mainmenu&amp;type=${admin_menu_type}&amp;id=${admin_menu['id']}">
      ${admin_menu_status}
    </a>
  </td>
  <td align="center">
    <a href="admin.php?mode=admin&amp;action=mainmenu&amp;type=${admin_header_type}&amp;id=${admin_menu['id']}">
      ${admin_header_status}
    </a>
  </td>
</tr>
