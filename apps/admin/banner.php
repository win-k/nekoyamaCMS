...
// アクセス制限
  ...
  if(ereg("admin/banner.php", $_SERVER['PHP_SELF']))
  {
  ...
  if($input_type == "title")
  {
       ...
       ereg($PHP_SELF, $HTTP_REFERER))
    {
      $admin_query = "update wps_menu set display_name='${input_name}',"
                   . "active='${input_active}'"
                   . " where internal_name='menu_banner'";
      mysql_query($admin_query, $db);
    }
    header("Location: admin.php?mode=admin&action=banner");
  }
...
// メニューアイテムの追加
  ...
  elseif($input_type == "new")
  {
    if(strtoupper($REQUEST_METHOD) == "POST" &&
       ereg($PHP_SELF, $HTTP_REFERER))
    {
      $admin_imgage_size = @getimagesize("./images/${input_image}");
      $admin_item = '<center><a href="' . $input_url
                  . '" target="_blank"><img src="images/'
                  . $input_image . '" ' . $admin_image_size[3]
                  . ' border="0"></a><br></center>';

      $admin_query = "select max(weight) from wps_menu_banner";
      $admin_result = mysql_query($admin_query, $db);
      $admin_weight = mysql_result($admin_result, 0, 0);
      $admin_weight++;

      $admin_query = "insert into wps_menu_banner"
      ...
      {
        $admin_weight++;
        $admin_item = '<img src="images/bk.gif" width="1" height="8"><br>';
        $admin_query = "insert into wps_menu_banner"
                     . "(name,item,active,weight,header)"
                     . " values('-','${admin_item}',"
                     . "'${input_active}',${admin_weight},'N')";
...
// 表示順の変更
  ...
  elseif(preg_match("/^move/", $input_type))
  {
    if(ereg($PHP_SELF, $HTTP_REFERER))
    {
      ...
      // 前後のカテゴリを移動
      $admin_query = "update wps_menu_banner set weight=${admin_back}"
                   . " where weight=${admin_weight}";
      mysql_query($admin_query, $db);

      // 編集中のカテゴリを移動
      $admin_query = "update wps_menu_banner set weight=${admin_forward}"
                   . " where id=${input_id}";
      mysql_query($admin_query, $db);
    }
    header("Location: admin.php?mode=admin&action=banner");
...
// 表示順の重複解消
  ...
  elseif($input_type == "conflict")
  {
    if(ereg($PHP_SELF, $HTTP_REFERER))
    {
      $admin_query = "select id from wps_menu_banner"
                   . " order by weight";
      $admin_result = mysql_query($admin_query, $db);
      $admin_menu_data = mysql_result($admin_result, 0, 0);
      $admin_weight = 1;
      while($admin_menu = mysql_fetch_assoc($admin_menu_data))
      {
        $admin_query = "update wps_menu_banner"
...
// メニュー項目の有効化
  ...
  elseif($input_type == "activate")
  {
    if(ereg($PHP_SELF, $HTTP_REFERER))
    {
      $admin_query = "update wps_menu_banner set active='Y'"
                   . " where id=${input_id}";
      mysql_query($admin_query, $db);
    }
    header("Location: admin.php?mode=admin&action=banner");
...
// メニュー項目の無効化
  ...
  elseif($input_type == "deactivate")
  {
    if(ereg($PHP_SELF, $HTTP_REFERER))
    {
      $admin_query = "update wps_menu_banner set active='N'"
                   . " where id=${input_id}";
      mysql_query($admin_query, $db);
    }
    header("Location: admin.php?mode=admin&action=banner");
...
// ヘッダーの有効化
  ...
  elseif($input_type == "show")
  {
    if(ereg($PHP_SELF, $HTTP_REFERER))
    {
      $admin_query = "update wps_menu_banner set header='Y'"
                   . " where id=${input_id}";
      mysql_query($admin_query, $db);
    }
    header("Location: admin.php?mode=admin&action=banner");
...
// ヘッダーの無効化
  ...
  elseif($input_type == "hide")
  {
    if(ereg($PHP_SELF, $HTTP_REFERER))
    {
      $admin_query = "update wps_menu_banner set header='N'"
                   . " where id=${input_id}";
      mysql_query($admin_query, $db);
    }
    header("Location: admin.php?mode=admin&action=banner");
...
// メニューアイテムの編集
  ...
  elseif($input_type == "edit")
  {
    if(ereg($PHP_SELF, $HTTP_REFERER))
    {
      ...
      $admin_query = "update wps_menu_banner set name='${input_name}',"
...
// メニューアイテムの削除
  ...
  elseif($input_type == "delete")
  {
    if(ereg($PHP_SELF, $HTTP_REFERER))
    {
      $admin_query = "delete from wps_menu_banner where id=${input_id}";
      mysql_query($admin_query, $db);
    }
  }
...
&nbsp;&nbsp;[&nbsp;
<a href="admin.php?mode=admin&amp;action=banner&amp;type=delete&amp;id=${input_id}">
はい
</a>
&nbsp;/&nbsp;
...
_EOT_;
  ...
  {
    $admin_query = "select * from wps_menu_banner"
                 . " where id=${input_id}";
...
<tr>
  <td align="center">
<a href="admin.php?mode=admin&amp;action=banner&amp;type=conflict">
順位
</a>
  </td>
...
_EOT_;
  ...
  $admin_query = "select * from wps_menu_banner order by weight";
...
_EOT_;
    ...
    {
      $page_content .= <<<_EOT_
<a href="admin.php?mode=admin&amp;action=banner&amp;type=moveup&amp;id=${admin_menu['id']}&amp;weight=${admin_menu['weight']}">
▲
</a>
_EOT_;
    }
    ...
    {
      $page_content .= <<<_EOT_
<a href="admin.php?mode=admin&amp;action=banner&amp;type=movedown&amp;id=${admin_menu['id']}&amp;weight=${admin_menu['weight']}">
▼
</a>
_EOT_;
    }
  ...
  <td align="center">
    <a href="admin.php?mode=admin&amp;action=banner&amp;type=${admin_menu_type}&amp;id=${admin_menu['id']}">
      ${admin_menu_status}
    </a>
  </td>
  <td align="center">
    <a href="admin.php?mode=admin&amp;action=banner&amp;type=${admin_header_type}&amp;id=${admin_menu['id']}">
      ${admin_header_status}
    </a>
  </td>
  <td align="center">
_EOT_;
      ...
      {
        $page_content .= <<<_EOT_
    <a href="admin.php?mode=admin&amp;action=banner&amp;type=editview&amp;id=${admin_menu['id']}">
      編集
    </a>
_EOT_;
      }
  ...
  <td align="center">
    <a href="admin.php?mode=admin&amp;action=banner&amp;type=confirm&amp;id=${admin_menu['id']}">
      削除
    </a>
  </td>
</tr>
